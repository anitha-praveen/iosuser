...
@objcMembers public class MTSlideToOpenView: UIView {
    ...
    public var appLanguageEnglish: Bool = true {
        didSet {
            appLanguageEnglishValue = appLanguageEnglish
        }
    }
    ...
    private var appLanguageEnglishValue: Bool?
    ...
    @objc private func handlePanGesture(_ sender: UIPanGestureRecognizer) {
        ...
        var value = CGFloat()
        if appLanguageEnglishValue == true {
            value = 1
        } else {
            value = -1
        }
        let translatedPoint = sender.translation(in: view).x * value
        switch sender.state {
        case .began:
            break
        case .changed:
